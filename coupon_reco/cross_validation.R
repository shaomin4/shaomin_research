...
FuAPk <- function (uNumToExamineK, vbYreal, vdYhat){
  ...
  uAveragePrecision <- sum(cumsum(vbYreal_sort_d) * vbYreal_sort_d / seq_along(vbYreal_sort_d)) /
    sum(vbYreal_sort_d)
  ifelse(is.nan(uAveragePrecision), 0, uAveragePrecision)
}
...
# Split training coupon into 10-folds
n <- 10
nr <- length(training_coupon_id)
fld_id <- split(sample(x = seq(nr), size = nr, replace = FALSE),
                rep(seq(n), each=1, length.out=nr))
for(fold in seq_along(fld_id)) {
  
  # New training set
  coupon_list_train_df <- dat_list$`~/coupon_reco/data/coupon_list_train.csv`[setdiff(seq(nrow(dat_list$`~/coupon_reco/data/coupon_list_train.csv`)), fld_id[[fold]]), ]
  # New validation set
  coupon_list_test_df <- dat_list$`~/coupon_reco/data/coupon_list_train.csv`[fld_id[[fold]],]
  # Get user data (all)
  ...
  area_train = dat_list$`~/coupon_reco/data/coupon_area_train.csv`[dat_list$`~/coupon_reco/data/coupon_area_train.csv`$COUPON_ID_hash %in% coupon_list_train_df$COUPON_ID_hash, ]
  # Get test coupon's area data
  area_test = dat_list$`~/coupon_reco/data/coupon_area_train.csv`[dat_list$`~/coupon_reco/data/coupon_area_train.csv`$COUPON_ID_hash %in% coupon_list_test_df$COUPON_ID_hash, ]
  # Get train purchase log 
  purchase_log_train_df <- dat_list$`~/coupon_reco/data/coupon_detail_train.csv`[dat_list$`~/coupon_reco/data/coupon_detail_train.csv`$COUPON_ID_hash %in% coupon_list_train_df$COUPON_ID_hash, ]
  # Get test purchase log 
  purchase_log_test_df <- dat_list$`~/coupon_reco/data/coupon_detail_train.csv`[dat_list$`~/coupon_reco/data/coupon_detail_train.csv`$COUPON_ID_hash %in% coupon_list_test_df$COUPON_ID_hash, ]
  ...
  purchase_sparse_train_mtx <- sparseMatrix(i = sapply(purchase_log_train_df$USER_ID_hash, function(X) which(X == user_train_id)),
                                            ...
                                            x = ifelse(purchase_log_train_df$ITEM_COUNT > 0, 1, 0),
                                            dims = c(length(user_train_id), length(coupon_train_id)),
                                            dimnames = list(user_train_id, coupon_train_id))
  purchase_sparse_test_mtx <- sparseMatrix(i = sapply(purchase_log_test_df$USER_ID_hash, function(X) which(X == user_train_id)),
                                           j = sapply(purchase_log_test_df$COUPON_ID_hash, function(X) which(X == coupon_test_id)),
                                           x = ifelse(purchase_log_test_df$ITEM_COUNT > 0, 1, 0),
                                           dims = c(length(user_train_id), length(coupon_test_id)),
                                           dimnames = list(user_train_id, coupon_test_id))
  ...
  for(i in seq(r)) {
    k <- r[[i]]
    print(k)
    purchase_nmf <- nmf(data = purchase_sparse_train_mtx, k, algorithm="mu", initMethod="EIn-NMF", maxiter = 500, runs = 5, verbose = T)
    save(purchase_nmf, file = paste0("~/coupon_reco/data/purchase_nmf/purchase_nmf_", k, ".RData"))
  }
  ### read nmf RData
  purchase_NMF_list <-list()
  for(i in seq(r)){
    purchase_NMF_list[[i]] <- get(load(paste0("~/coupon_reco/data/purchase_nmf/purchase_nmf_", r[i], ".RData")))
  }
  ...
  for(i in seq(r)) {
    k <- r[[i]]
    print(k)
    coupon_train_nmf <- nmf(data = t(coupon_list_train_mtx), k, algorithm="mu", initMethod="EIn-NMF", maxiter = 500, runs = 5, verbose = T)
    save(coupon_train_nmf, file = paste0("~/coupon_reco/coupon_train/coupon_train_nmf_", k, ".RData"))
  }
  
  # Read NMF RData
  train_coupon_NMF_list <-list()
  for(i in seq(r)){
    train_coupon_NMF_list[[i]] <- get(load(paste0("~/coupon_reco/coupon_train/coupon_train_nmf_", r[i], ".RData")))
  }
  ...
  #NMF encoding
  encoding.train <- t(predict(train_coupon_NMF_list[[coupon_NMF_idx]]))
  encoding.test <- t(predict(train_coupon_NMF_list[[coupon_NMF_idx]], t(coupon_list_test_mtx)))
  ...
  # Get purchase NMF model
  purchase_nmf <- purchase_NMF_list[[purchase_NMF_idx]]
  ...
  MAP_result <- sapply(X = seq(nrow(purchase_sparse_test_mtx)), FUN = function(idx) {
    FuAPk(uNumToExamineK = 10, vbYreal = purchase_sparse_test_mtx[idx, ], vdYhat = purchase_test_mtx[idx, ])
  })
  MAP_result <- mean(MAP_result)
  
}

FuAPk(uNumToExamineK = 10, vbYreal = purchase_sparse_test_mtx[1, ], vdYhat = purchase_test_mtx[1, ])
