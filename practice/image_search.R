...
food101_dir = "/data_lake/food-101"
...
image_folders = list.files(paste(food101_dir,"/images", sep = ""), full.names = F)
# All test image file path
img_test = list()
for(f in image_folders){
  img_test = append(img_test,list.files((paste(food101_dir, "/test/",f, sep = "")), recursive = TRUE, full.names = TRUE))
}
...
#save(predictions,file = "~/RProject/food101/predictions_feature_softmax100.RData")
#load("~/RProject/food101/predictions_feature_fintune.RData")

# Get test image feature
img_path <- "/data_lake/food-101/train/sushi/100332.jpg"
